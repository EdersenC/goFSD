...
var ErrInvalidSampleAge = errors.New("invalid stop-sign motion-plan sample age")
...
func ValidatePlan(plan Plan) error {
	if plan.Contract != StopSignMotionPlanContractV1 {
		return planError("contract %q must equal %q", plan.Contract, StopSignMotionPlanContractV1)
	}
	if plan.Direction != StopSignDirectionForward {
		return planError("direction %q must equal %q", plan.Direction, StopSignDirectionForward)
	}
	...
	for index, point := range plan.Points {
		...
		if !finite(point.DesiredSpeedMPS) || point.DesiredSpeedMPS < 0 || point.DesiredSpeedMPS > StopSignMotionPlanMaxSpeedMPS {
			return planError("point %d desired_speed_mps %.6f is outside [0, %.2f]", index, point.DesiredSpeedMPS, StopSignMotionPlanMaxSpeedMPS)
		}
...
func (s *PlanSampler) SampleByAge(ageMs float64) (Setpoint, error) {
	...
	panic("validated stop-sign motion-plan sampler failed to cover an in-range age")
}
