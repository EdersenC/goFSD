...
func validPlan() Plan {
	return Plan{
		Contract:    ParkingSetpointContractV1,
		SampledAtS:  100,
		ReceivedAtS: 100.025,
		Direction:   ParkingDirectionForward,
		Points: []Setpoint{
...
func TestValidatePlanRejectsContractTimestampOrderingAndPointBounds(t *testing.T) {
	tests := []struct {
		...
		{name: "wrong contract", mutate: func(plan *Plan) { plan.Contract = "parking_setpoint_v2" }},
		...
		{name: "speed above contract maximum", mutate: func(plan *Plan) { plan.Points[0].DesiredSpeedMPS = ParkingSetpointMaxSpeedMPS + 0.001 }},
...
func TestPlanSamplerOwnsPlanCopy(t *testing.T) {
	...
	plan.Points[0].DesiredSpeedMPS = ParkingSetpointMaxSpeedMPS
