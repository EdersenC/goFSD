...
class StopSignControlContractTests(unittest.TestCase):
    ...
    def test_dataset_targets_ignore_steering_and_raw_actuation(self) -> None:
        targets = build_control_targets({
            "expertDesiredWheelSteerNormalized": -0.9,
    ...
    def test_config_rejects_legacy_parking_and_actuator_outputs(self) -> None:
        for names in (
            ["desired_wheel_steer_normalized", "desired_speed_mps", "stop_probability"],
            ["steering", "acceleration", "brakePressureAvg"],
        ):
    ...
    def test_checkpoint_and_predict_require_exact_contract(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, CONTROL_CONTRACT_NAME):
            require_predict_control_contract({"control_contract": "parking_setpoint_v1"})
