...
- Small behavior changes that scale with `motionVariancePct`: braking can become only slightly harder (`3.80–4.05 m/s²` at the 25% maximum), while release acceleration can become moderately faster (`3.50–4.20 m/s²`). The generator never creates softer braking or a slower-than-standard release, limiting contradictory imitation targets. The changed desired-speed trajectory drives the physical replay; expert throttle/brake remain derived diagnostics.
